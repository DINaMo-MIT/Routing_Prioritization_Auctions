...
def final_plotter(output, axs, scenario = "NULL", title= True):
    ...
    sns.scatterplot(x=np.array(data_avg_del), y=data_avg_std, hue=methods, ax=axs[2], style=methods, s=icon_size, legend=title)
    ...
    sns.scatterplot(x=np.array(data_avg_wait), y=data_avg_std_weighted, hue=methods, ax=axs[4], style=methods,  s=icon_size, legend=title)
